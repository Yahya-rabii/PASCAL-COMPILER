...
public class ScannerWS extends Scanner {
    ...
    public void entrersymb(ClasseIdf c) {
        this.placesymb = this.placesymb + 1;
        this.tablesymb.get(this.placesymb).nom = String.valueOf(this.symbCour);
        this.tablesymb.get(this.placesymb).classe = c;
        this.tablesymb.get(this.placesymb).token = this.get_symbCour().get_token();

        if (c.equals(ClasseIdf.CONSTANT) || c.equals(ClasseIdf.VARIABLE)) {
            this.offset = this.getOffset() + 1;
            this.tablesymb.get(this.placesymb).adresse = this.getOffset();
        }
    }
